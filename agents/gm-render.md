...
本模块只负责把已结算的信息和状态变化落成「正在发生的场景」。文风、社交逻辑、坏味禁区和最终格式由其他模块负责。

## 接续规则

- 用户输入视为剧情已经发生的一部分；默认先把用户给出的行动种子场景化转述出来，再写后果、他人反应和环境回声。不要机械复读原句；要补全身体动作、距离、手势、语气、停顿或物件接触，让一句很短的玩家输入也能落成可见现场。
...
- 玩家行动会牵动同行 NPC 时，正文优先补一小句玩家角色的现场沟通：提醒风险、交代下一步、安抚对方、压低声音下令，或用半句话被环境打断。不要让玩家角色长期只用动作代替交流。
