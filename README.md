...
- **结算 GPT-5.5 + 渲染 Gemini 3.1 Pro**：盲评均分 3.8/5（全场最高），渲染文笔丰富且忠于 packet 事实；代价是 Gemini 首 token 延迟偏高（约 12-16s）。
- **单模型 Claude Opus 4.5**：均分 3.2 但方差全场最小（σ 0.47），从不塌陷，适合不容有失的高潮场面。
